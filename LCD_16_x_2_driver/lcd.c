...
static void LCD_Write(bool regSelect, uint8_t byte)
{
	...
	*/
	...
	GPIO_Output_Reg_Write(GPIOC, (byte >> 4) << 4);
	...
	//Send Lower Nibble
	GPIO_Output_Clear_Reg(GPIOC, ((1<<LCD_D4) |
																(1<<LCD_D5) |
																(1<<LCD_D6) |
																(1<<LCD_D7)));
																
	GPIO_Output_Reg_Write(GPIOC, (lowNibble >> 4) << 4);
...
void LCD_Init(void)
{
	...
	*/
	...
	//Configure PC4...7 as output
	GPIO_Output_Init(GPIOC, (GPIO_PIN4_GEN_PUR_OUTPUT | GPIO_PIN5_GEN_PUR_OUTPUT |
	...
	LCD_Write(LCD_CMD_SELECT, CMD_FUNCTION_SET_4BIT );
